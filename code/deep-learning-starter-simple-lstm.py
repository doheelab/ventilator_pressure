...
def plot_sample(sample_id, df):
    ...
    cols = (
        ["u_in", "u_out", "pressure"] if "pressure" in df.columns else ["u_in", "u_out"]
    )
...
# (config, df_train, df_val, df_test, fold) = (config, df_train, df_val, df_test, i)
...
def train(config, df_train, df_val, df_test, fold):
    ...
    pred_test = predict(
        model, test_dataset, batch_size=config.val_bs, device=config.device
    )
